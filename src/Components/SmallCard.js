...
function SmallCard({res,id}){                  
   let [error,setError]=useState(false);
   let {data,fetchError}=useWeatherInfo({city:res,component:"SmallCard"});
      ...
      <div className="flex flex-row w-52 items-center justify-between">
        <div className="flex items-center">
            ...
            (<img className="h-9 w-9" 
             ...
             alt={data.weather[0].description && data.weather[0].description}
             onError={()=>{setError(true)}}
             ></img>):
             null}
             <h1>{data.main.temp && Math.floor(data.main.temp-273.15)}°C</h1>
            </div>
            ...
            <h1>{`${res[0].toUpperCase()}${res.slice(1).toLowerCase()}`}</h1>
            </div>
      </div>
      <h1 className="font-light text-sm">{`${data.weather[0]["description"] && data.weather[0]["description"][0].toUpperCase()+data.weather[0]["description"].slice(1)}`}</h1>
